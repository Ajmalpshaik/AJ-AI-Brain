...
| [`brain-update-layers`](skills/brain-update-layers/SKILL.md) | "Update the brain" / end of day / after a `git pull` / a search feels stale — refresh the search index, knowledge graph and Obsidian vault, and prove they are current |
...
- `tools/` — `fragment-index.mjs` (**search the 398 fragments instead of reading the index** — `--find <word>` matches purpose and input fields and shows each hit's proven status, `--show <path>` prints what one fragment needs filled in, `--json` for other tools; computed from the fragments themselves so it can't go stale. Serves the library's own rule: reuse a fragment when one fits, only write new C# when none does), `brain-status.mjs` (**one honest answer to "what is the state of this Brain?"** — counts, how much of the library has actually been run against a real model, open items, drift; computed from disk every time so it can't go stale, and wired as a SessionStart hook so a fresh session knows before it acts. `--full`, `--capabilities`, `--json`), `verify-fragments-compile.ps1` (**compile-checks all 398 C# fragments against the real Revit API DLLs without opening Revit** — catches the misspelled-member/wrong-overload/wrong-version class of bug that a fragment can otherwise carry for months if it is never executed; needs Revit installed and a Roslyn `csc.exe`, and `-DryRun` validates the harness itself on a machine with neither), `invoke-bridge.ps1` (fallback bridge caller) and `verify-consistency.ps1` (drift checks over this whole folder: skill frontmatter, markdown link targets, scripts-README-vs-disk, skill coverage in the entry docs, AGENT-SPEC's fragment counts, text encoding, fragment `// SOURCE:` cross-references, semantic-index coverage claims, and live counts stated anywhere in markdown; `verify-consistency.mjs` is the portable Node version and is the one the hook runs — it is the authority, and the PowerShell copy trails it; `verify-consistency-hook.mjs` wraps it as the auto-run edit hook wired in `.claude/settings.json` — it runs the Node checker because the older `verify-consistency-hook.ps1` fired on Windows only and silently did nothing on Linux/macOS or Claude Code for web, which is exactly where an agent is least likely to run it by hand)
- `semantic-index/` — **ask this Brain a question in plain English instead of knowing where to look.** `ask-brain-hybrid.cmd "how do I stop ducts clashing the ceiling"` searches all 479 files (`skills/`, `knowledge/`, `scripts/`, root docs, and the native-tools reference `mcp-server/tools/README.md`) by *meaning* as well as exact words, and returns real paths. It complements `tools/fragment-index.mjs` rather than replacing it: that one is keyword-only and reads `scripts/*.cs` only, so it structurally cannot surface the skill or knowledge note that answers a question — and it needs you to already know the keyword. Python + a local ChromaDB, offline after a one-time model download, and writes nothing to `%TEMP%` (see [`semantic-index/README.md`](semantic-index/README.md) — that constraint is deliberate, not incidental). **It used to go stale silently; since 2026-08-13 it rebuilds itself** at the end of any session turn that edited a file (`tools/reindex-mark.mjs` + `tools/reindex-run.mjs`). Edits made *outside* a session — a git checkout, a file changed in an editor — still need `index-brain.cmd` by hand (2–4 s; it only re-reads what changed). Only its source is committed; the model and database are git-ignored. **Read the top 3–5, not just #1** — the measured accuracy lives in [`semantic-index/score-history.md`](semantic-index/score-history.md), one stamped line per run (last run, 2026-08-25: 3/28 at #1, 8/28 in top 3, 11/28 in top 5, 20/28 retrievable, MRR 0.228 — the corpus nearly doubled since the 2026-08-21 runs, so lines on different corpus sizes are not comparable). Quote that file rather than a remembered number. The misses are site-vocabulary ones ("floor levels" finds the slab creator, not `create-levels.cs`), so try the Revit term when a result looks off
- **What runs by itself at the end of every session turn** — [`tools/stop-hooks.mjs`](tools/stop-hooks.mjs)
  fans four jobs out in parallel, so none of them depends on anyone remembering:
  [`reindex-run.mjs`](tools/reindex-run.mjs) rebuilds the semantic index if a file changed,
  [`score-check.mjs`](tools/score-check.mjs) re-scores the search,
  [`test-row-nudge.mjs`](tools/test-row-nudge.mjs) asks for a test question when the set is short, and
  [`fragment-nudge.mjs`](tools/fragment-nudge.mjs) **says out loud when fresh C# was written instead of
  composed from the library, and when nothing from a session was saved back.** That last one exists
  because on 2026-08-13 thirteen `run_csharp` calls in one real session used **zero** saved fragments and
  saved **nothing** back — both halves of this Brain's promise skipped at once, invisibly, while the job
  log had recorded every call the whole time. The lesson it encodes: *a step that depends on remembering
...
- `.claude-plugin/` — makes this whole repo installable as a Claude Code **plugin** (all 12 skills + the MCP relay in one install — see [`SETUP.md`](SETUP.md) step 1, Option A)
