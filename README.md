...
- `semantic-index/` — **ask this Brain a question in plain English instead of knowing where to look.** `ask-brain-hybrid.cmd "how do I stop ducts clashing the ceiling"` searches all 314 files (`skills/`, `knowledge/`, `scripts/`, root docs, and the native-tools reference `mcp-server/tools/README.md`) by *meaning* as well as exact words, and returns real paths. It complements `tools/fragment-index.mjs` rather than replacing it: that one is keyword-only and reads `scripts/*.cs` only, so it structurally cannot surface the skill or knowledge note that answers a question — and it needs you to already know the keyword. Python + a local ChromaDB, offline after a one-time model download, and writes nothing to `%TEMP%` (see [`semantic-index/README.md`](semantic-index/README.md) — that constraint is deliberate, not incidental). **Unlike everything else in `tools/`, this one CAN go stale**: it is a snapshot, so run `index-brain.cmd` (~80 s) after adding anything to the Brain. Only its source is committed; the model and database are git-ignored. **Read the top 3–5, not just #1** — measured against 24 independently-written questions in a modeller's own words, #1 was right or useful about three-quarters of the time, and the misses are site-vocabulary ones ("floor levels" finds the slab creator, not `create-levels.cs`), so try the Revit term when a result looks off
- `mcp-server/` — the Node.js relay that talks to a Revit-side bridge listener (see [`SETUP.md`](SETUP.md) step 2)
- `.claude-plugin/` — makes this whole repo installable as a Claude Code **plugin** (all 10 skills + the MCP relay in one install — see [`SETUP.md`](SETUP.md) step 1, Option A)
