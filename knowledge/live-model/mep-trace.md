...
**This is a general method, parameterized by pipe/system type — never hardcode it to one system.** The
user will name a different pipe type almost every time they ask ("refrigerant", "condensate drain",
"water supply" — it changes per request, so the system-name filter is always an input, not a fixed
constant). Check `glossary.md` for the mapping from the user's word to *this project's* actual
Revit system-type name(s) to filter on — a project's naming convention can group several related
sub-types under one umbrella term (e.g. "refrigerant" might cover a whole family of system names sharing
a prefix) — don't assume the filter is only the one or two names you'd guess; confirm the real list
against the model.

**Preferred approach — bulk clustering, not one path at a time:** rather than tracing a single named
starting unit outward, process the *whole* filtered pipe/fitting set at once and let the circuits fall
out of the geometry themselves:
...
4. For each open end, identify what's actually there: match against known equipment connector positions
   (for equipment with a `ConnectorManager`) or, if that equipment has no connectors at all (some
   outdoor/condensing-style equipment has none — check first, don't assume), match to the nearest
   equipment by bounding-box distance instead.
...
**Why bulk clustering over one-path-at-a-time:** the user doesn't have to name a specific starting unit —
they just name the pipe/system type they care about, and every circuit of that type gets traced and
reported in one pass. Faster when there are many circuits, and matches how a modeler actually thinks
about the problem: gather all pipes of the type → walk each pipe's two ends → merge whatever touches →
repeat until every set is complete → whatever equipment sits at a set's ends is what's really connected.

The original hop-by-hop, single-path version of this (step 2's fallback, walked manually one connector at
a time rather than as a bulk merge) is the same underlying technique — cap the hop count (a few dozen is
usually plenty for a real chain) to avoid an infinite loop if the model has a genuine geometric coincidence
that isn't a real path.

**Don't assume tag/name conventions reflect actual connectivity** — always verify by tracing before
reporting a connection as fact. Equipment that looks like it should pair by matching code or name can
turn out to be cross-connected, redundant, or wired in some other way the naming doesn't hint at — the
naming convention can be actively misleading, not just incomplete. Treat any pairing implied by names/tags
as a hypothesis to verify geometrically, never as the answer itself.
...
then `SetSurfaceForegroundPatternColor`/`Id`/`Visible` and the matching `Cut...` triple). Line-color-only
generally reads as insufficient — most users expect the fill to change too.
