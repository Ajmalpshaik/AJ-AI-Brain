...
- Conclusion: don't spend time re-attempting this. To exercise a real compiled add-in command's behavior
  from a script, replicate the needed logic directly with plain Revit API calls, or ask the user to click
  the button themselves once preconditions (e.g. view visibility) are set up.

## Tagging directly via script instead of clicking a real AJ Tools command
- the user explicitly rejected "just click Smart MEP Tags yourself" and asked for direct AI-driven tagging
...
- **What could NOT be reproduced**: `SmartTagPlacementEngine`'s real clash-free scoring/placement — that
  ...
  overlap. Tell the user this plainly every time this recipe is used — don't imply clash-free placement it
  didn't actually do. If the user wants the exact scored placement without clicking the button themselves,
  that needs a genuinely new compiled add-in command, not a bigger script — out of scope for this Brain.
...
- **Proof case**: tagging a batch of ducts, picked "M_Duct Size Tag" because it's the generic out-of-box
  Autodesk name and looked like the safe default. The user corrected this: they wanted whatever tag family
  Revit itself uses when tagging a duct manually — not a guess, not any add-in's own tag-settings dialog
  (a separate, add-in-specific priority/enable setting), Revit's own per-category default.
  ...
  **not** have per-MEP-category tag entries (no `DuctTagType` etc. — checked the full enum list, only
  ...
  is "whatever Tag by Category would use", and it returns whatever custom project tag family that
  project has actually standardized on, not necessarily the generic Autodesk one. Use this lookup (per
  relevant `OST_..Tags` category) instead of hardcoding or guessing a family name whenever the user asks
  for "the [normal/standard/manual] tag", across any category (pipe, equipment, cable tray).
...
- **Proof case**: the user reported "the leader is clashing with its own tag" after the first live-tagged
...
- **Also true for `SmartMepTagService`'s own compiled placement logic** — not yet checked whether it has
  the same category of bug (using a guard-only constant as a clearance distance). Worth a look if the user
...
- **Different problem from the own-leader clash above**: that one was a tag's leader crossing into its
  ...
  close together both landing tags nearby). the user asked directly: "any idea to fix this issue" — the
...
- **This is NOT full clash-free placement** — it only resolves what's already overlapping after initial
  ...
  real `SmartMepTagService`/Intelligent Tag Arranger (clicked by the user) remains the actual clash-free
  tool.
...
- **Proof case**: after switching to fixed-side placement (below for horizontal ducts), the user reported
...
- **Evolution of the same "which side" question across this session** (each version replaced the last):
  1. Alternate left/right by index (anti-clash spread) — the user: looked inconsistent/messy.
  ...
  4. Nearest-riser-by-X-proximity — fixed the multi-cluster case (each branch finds its own local
     riser), but still just a geometric proxy for what the user actually meant.
  5. **Real flow direction** (final, confirmed with the user directly asking "is it based on where flow is
...
- **After confirming the registry-based rebuild looked "perfect"**, the user pointed out across
  color-coded views (blue=Supply, magenta=Return, green=Exhaust) that Return and Exhaust leaders
  correctly followed real flow direction, and Supply ALSO correctly followed real flow direction — but
  they wanted Supply to be the OPPOSITE, "same like a supply and return" (the standard drafting
  convention of drawing supply/return as visual mirror images for quick distinction, independent of what
  each system's own real flow direction says). Asked one clarifying question before rebuilding —
  which system(s) should flip — confirmed: Supply only; Return/Exhaust stay as they were.
...
- **the user, 2026-07-14**: asked directly for an improved design, pointed at AJ Tools' own compiled
...
- **the user's rule, stated directly after the flow-direction/clash trade-off discussion**: "before start
...
- **Third clash type the user found, same session**: after fixing own-leader-clash and tag-vs-tag overlap
  separately, they reported "tag clash with duct" — a tag's text box sitting on top of duct geometry
