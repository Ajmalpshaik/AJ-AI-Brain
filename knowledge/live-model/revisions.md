...
- **Root cause (best evidence, not confirmed by Autodesk docs)**: any Revision never referenced by a
  ...
  In this case the user confirmed the two orphans were already stale/unwanted (their own prior cleanup), so
  no recovery was needed — but treat this as a real, silent, project-wide side effect every time, not a
  one-off. **Before creating a Revision meant to persist without being placed on a sheet immediately**,
  either attach it to at least one sheet in the same transaction, or warn the user it may not survive the
