...
| **"It said it failed but the model changed anyway"**, or an error that says only *"could not roll back"* — a rollback or a post-commit refresh throwing and burying the real error | [`failure-handling-without-a-class.md`](failure-handling-without-a-class.md) |
