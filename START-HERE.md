...
| **You don't know which row above applies**, or don't know the word to grep for | `semantic-index\ask-brain-hybrid.cmd "the request, in plain English"` — searches all 350 files by meaning *and* exact words. **Read the top 3–5, not just #1** (last reproducible run: 3/14 at #1, 5/14 in top 3 — quote [`semantic-index/score-history.md`](semantic-index/score-history.md), never a remembered figure); weakest on site vocabulary, so try the Revit word too — see [`semantic-index/README.md`](semantic-index/README.md) |
