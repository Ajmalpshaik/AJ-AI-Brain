...
| **You don't know which row above applies**, or don't know the word to grep for | `semantic-index\ask-brain-hybrid.cmd "the request, in plain English"` — searches all 478 files by meaning *and* exact words. **Read the top 3–5, not just #1** (last run, 2026-08-25, on the 28-row set: 3/28 at #1, 8/28 in top 3, 20/28 found at all — quote [`semantic-index/score-history.md`](semantic-index/score-history.md), never a remembered figure); weakest on site vocabulary, so try the Revit word too — see [`semantic-index/README.md`](semantic-index/README.md) |
