...
The 398 fragments in `scripts/` were proven one at a time against a real model. Until this tool, none of
...
now also have a native tool above), and the 398 real C# fragments in `scripts/` (51 filters, 243
actions, 36 creators, 8 commands, 44 recipes, 3 examples, 12 read-only `context/` fragments, 1 shared
