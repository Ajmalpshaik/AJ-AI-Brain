...
now also have a native tool above), and the 270 real C# fragments in `scripts/` (49 filters, 144
actions, 33 creators, 8 commands, 22 recipes, 3 examples, 10 read-only `context/` fragments, 1 shared
`lib/` prelude — count re-verified 2026-08-13, and now enforced by `tools/verify-consistency.*` check 5 so it cannot drift
