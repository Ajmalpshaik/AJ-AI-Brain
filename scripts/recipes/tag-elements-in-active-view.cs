...
//          AJ Tools' own compiled SmartTagPlacementEngine.cs (the user, 2026-07-14: "you can refer our
...
// half the tags to converge, which silently destroyed the flow-direction correctness the user had already
...
// does it fall back to the least-overlapping candidate — "tag everywhere" (the user's stated goal) means
...
// class. If the user ever wants the exact compiled engine run without clicking the button himself, that
...
string tagFamilyTypeNameOverride = null;   // leave null to use Document.GetDefaultFamilyTypeId (the user's default)
...
bool onlyHorizontalRuns = true;            // true = skip vertical risers (the user's standing rule, 2026-07-13)
double minLengthMm = 1000.0;               // skip runs shorter than this (the user's standing rule, 2026-07-13)
...
// MANDATORY FIRST STEP (the user, 2026-07-13) — see gotcha above.
...
// real flow direction (the user, 2026-07-13, confirmed) — falls back to nearest-riser geometry only if a
...
// GOTCHA (the user, 2026-07-14): Supply tags must point OPPOSITE of real flow direction; Return and
...
using (var tx = new Transaction(doc, "Tag elements — scored candidate placement"))
{
    ...
    try
    {
        ...
        foreach (var duct in orderedDucts)
        {
            ...
            // a duct (the user: "I need to tag everywhere") — place the least-overlapping option.
            if (bestT1 == null)
            {
