...
| [`action-split-elements.cs`](actions/structural-changes/action-split-elements.cs) | Split each Duct/Pipe at a point along its own length (fraction or mm from start), auto-reconnects the joint — generalized from `recipes/split-duct-near-equipment.cs` — ✓ verified 2026-07-22 |
...
| [`recipes/split-duct-near-equipment.cs`](recipes/split-duct-near-equipment.cs) | Split a duct at a fixed gap from an equipment connector and reconnect the joint — NOT a standing default, only on explicit request — ✓ verified 2026-07-23 (BreakCurve Id-swap bug found+fixed, story in header) | `../knowledge/live-model/hvac-ducts.md` § Splitting an existing duct into two segments at a given point |
