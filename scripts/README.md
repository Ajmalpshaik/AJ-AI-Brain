...
| [`action-move-to-ray-hit.cs`](actions/move-copy-rotate/action-move-to-ray-hit.cs) | Fire ONE ray per element and move it to whatever the ray hits, plus a signed offset — "terminal up to the slab", "sprinkler up to the soffit", "bracket across to the wall". Direction AND target category are per-request inputs; dry-run by default. The general form of `recipes/ray-trace-to-ceiling.cs` — ray half proven, MOVE half needs a slab/ceiling fixture (2026-07-26) |
