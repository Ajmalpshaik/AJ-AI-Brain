...
| [`context/context-workset-info.cs`](context/context-workset-info.cs) | Worksharing on/off, and every user workset with open/closed state and owner — ✓ verified 2026-08-06 (non-workshared path only — the workset branch is still unproven) |
...
| [`context/context-linked-models.cs`](context/context-linked-models.cs) | Every RVT link — loaded/unloaded status, pinned, workset — orientation step before `filter-by-links.cs`/`filter-by-linked-model-elements.cs`/`action-reload-links.cs` — ✓ verified 2026-08-06 (no-links path only — the "links exist" branch is still unproven) |
