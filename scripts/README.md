...
| [`recipes/generate-room-coverage-layout.cs`](recipes/generate-room-coverage-layout.cs) | Fewest equal circles of a given radius that fully cover a room, drawn — the sprinkler/detector/CCTV layout question. Samples the real room shape, lays a hexagonal (or square) covering lattice, greedy-reduces it, then VERIFIES by counting uncovered sample points. Optional max-spacing code check, because full coverage is NOT compliance — ✓ verified 2026-07-26 (Room 4, 287.7 m²: r=3000 → 20 heads hexagonal / 21 square, 0 gaps; r=500 → 466, 0 gaps) | user's ask 2026-07-26 |
